...
Каждая запись показывает sender/recipient, попытку, локальное время и для response
его продолжительность. JSON результата форматируется только после `JSON.parse`,
а затем также вставляется через `textContent`.
