...
Таблица `agent_messages` хранит все делегации Coordinator и компактные ответы
Executor/Developer: направление, номер попытки, timestamp и длительность ответа.
Worker записывает request до вызова агента и response сразу после возврата. API
возвращает их в `task.agent_messages`, включая выполняющуюся задачу.
